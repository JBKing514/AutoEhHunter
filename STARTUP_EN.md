...
3.  **Import Workflow**:
    * Import `./Companion/n8nWorkflows/hunterAgent.json`.

4.  **Authentication Configuration**:
    * Configure Credentials for `OpenAI Chat Model` in n8n (pointing to your LLM backend), and select the model you want at `intent classifier` node.
    * In the `Call hunter_agent Skill` URL field of `hunter_agent`, enter the IP address and port number of the compute container.
    * Configure the Bot Tokens for the two `Telegram` nodes of `hunter_agent`.
    * Note that the `intent classifier` will call the new openAI `response` API; ensure that the selected backend supports this API.
