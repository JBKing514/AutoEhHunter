      ...
      <v-container fluid class="pa-6">
        <section v-show="tab === 'dashboard'">
          ...
          <v-row v-else>
            ...
            >
              <v-menu open-on-hover location="end" :open-delay="120" :close-delay="90" max-width="360">
                <template #activator="{ props }">
                  <v-card
                    v-bind="props"
                    class="home-card"
          ...
          <v-dialog :model-value="!!mobilePreviewItem" max-width="360" @update:model-value="onMobilePreviewToggle">
            <v-card v-if="mobilePreviewItem" class="pa-2 hover-preview-card" variant="flat">
              ...
              <div class="d-flex flex-wrap ga-1">
                <v-chip v-for="tag in itemHoverTags(mobilePreviewItem)" :key="`m-${mobilePreviewItem.id}-${tag}`" size="x-small" variant="outlined" class="hover-tag">{{ tag }}</v-chip>
              </div>
            </v-card>
          </v-dialog>
        ...
        <section v-show="tab === 'settings'">
          ...
          <v-card v-show="settingsTab === 'recommend'" class="pa-4 mb-4">
            ...
            <v-row>
              ...
              <v-col cols="12" md="4"><v-text-field v-model="config.REC_STRICTNESS" :label="labelFor('REC_STRICTNESS')" type="number" step="0.01" /></v-col>
              <v-col cols="12" md="4"><v-text-field v-model="config.REC_TAG_WEIGHT" :label="labelFor('REC_TAG_WEIGHT')" type="number" step="0.01" /></v-col>
              <v-col cols="12" md="4"><v-text-field v-model="config.REC_VISUAL_WEIGHT" :label="labelFor('REC_VISUAL_WEIGHT')" type="number" step="0.01" /></v-col>
...
async function runImageSearchQuick() {
  try {
    ...
    const res = await searchByImage({ arcid: refItem.arcid, scope: "both", limit: 24 });
