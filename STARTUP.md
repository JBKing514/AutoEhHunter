...
3.  **导入工作流**：
    * 导入 `./Companion/n8nWorkflows/hunterAgent.json`。

4.  **鉴权配置**：
    * 在 n8n 中配置 `OpenAI Chat Model` 的 Credentials (指向你的 LLM 后端)，并在`intent classifier`里选择你想指向的模型。
    * `hunter_agent`的`Call hunter_agent Skill`的URL栏里填写compute容器的IP地址:端口号
    * 配置`hunter_agent`的两个 `Telegram` 节点的 Bot Token。
    * 注意`intent classifier`会调用新的openAI `response` API，确保选择的后端支持该API。
